...
k_2 = sympy.Symbol("k_2")
alpha = sympy.Symbol("alpha")

# z=1-x-y
# инициализация уравнений
equation1 = k1 * x - k_1 * x - k3 * (1-y)**alpha*x*y
equation2 = k2 * (1-x-y) - k_2*y** 2 - k3 *(1- y )** alpha*x*y
# надоходим стационарные решения(при каких x и k1 уравнение равно 0)
# два решения
res = sympy.solve([equation1, equation2], x, k1, dict=True)
x_function = sympy.lambdify((y, k3, k_2), res[1][x])
k1_function = sympy.lambdify((y, k_1, k2, k3, k_2), res[1][k1])

A = sympy.Matrix([equation1, equation2])
jacobianMatrix = A.jacobian(sympy.Matrix([x, y]))
determinant = jacobianMatrix.det()
determinant_function = sympy.lambdify((x, y, k1, k2, k3, k_1, k_2), determinant)
trace = jacobianMatrix.trace()
trace_function = sympy.lambdify((x, y, k1, k2, k3, k_1, k_2), trace)
...
k1_y_init = det_y.subs(k2, init_k2).subs(k3,init_k3).subs(k_2,init_k_2).subs(k_1,init_k_1)


split_y = np.linspace(0.0001, 0.9999, num=1000)

x_res = x_function(split_y[0], init_k3, init_k_2)
k1_res = k1_function(split_y[0], init_k_1, init_k2, init_k3, init_k_2)
determinant_last = determinant_function(x_res, split_y[0], k1_res, init_k2, init_k3, init_k_1, init_k_2)
determinant_array = []
trace_array = []
x_array = []
k1_array = []
bifurcation_array = []
for i in split_y:
    x_res = x_function(i, init_k3, init_k_2)
    k1_res = k1_function(i, init_k_1, init_k2, init_k3, init_k_2)
    determinant_current = determinant_function(x_res, i, k1_res, init_k2, init_k3, init_k_1, init_k_2)
    ...
    trace_array.append(trace_function(x_res, i, k1_res, init_k2, init_k3, init_k_1, init_k_2))
...
for i in split_y:
    det_local_k_1 = det_k_1_on_y[0].subs(k2, init_k2).subs(k3,init_k3).subs(k_2,init_k_2).subs(y,i)
    tr_local_k_1 = tr_k_1_on_y[0].subs(k2, init_k2).subs(k3,init_k3).subs(k_2,init_k_2).subs(y,i)
    det_local_k1 = k1_function(i, det_local_k_1, init_k2, init_k3, init_k_2)
    tr_local_k1 = k1_function(i, tr_local_k_1, init_k2, init_k3, init_k_2)
...
def pend(y, t, ):
    par_x, par_y = y
    loc_eq1 = 0.03 * x - 0.01 * par_x - 10 * (1 - par_y) **alpha * par_x*par_y
    loc_eq2 = 0.05 * (1-par_x-par_y) ** 2 - 0.01 * par_y ** 2 - 10 * (1 - par_y) ** alpha * par_x * par_y
